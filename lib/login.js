 ...
 */
 ...
 function createToken (email, callback) {
   var self = this
   self.bcrypt.hash(email + self.secret, self.salt, function (err, hash) {
     if (err) {
       callback(err)
     } else {
       callback(null, hash)
 ...
 function checkToken (email, token, callback) {
   var self = this
   self.bcrypt.compare(email + self.secret, token, function (err, result) {
     if (err) {
       callback(err)
     } else {
 ...
 function reset (languageString, email, callback) {
   var self = this
   self.createToken(email, function (error, token) {
     if (error) {
       callback(error)
     } else {
       var mailOptions = {
         ...
         html: self.language.getString(languageString, 'reset_mail_text') + '<br/><a href="https://www.storytelling.de/set-pwd?resetToken=' + token + '&email=' + email + '">' + self.language.getString(languageString, 'reset_mail_button') + '</a>' // html body
       }
       ...
       self.database.addUser(dataset, function (err, user) {
         if (err) {
           ...
           var mailOptions = {
             ...
             subject: self.language.getString(user.language, 'register_mail_subject'), // Subject line
             html: self.language.getString(user.language, 'register_mail_text') + '<br/><a href="https://www.storytellingclub.de/app">' + self.language.getString(user.language, 'register_mail_button') + '</a><br/><table><tr><td>' + self.language.getString(user.language, 'login_language') + '</td><td>' + dataset.register_language + '</td></tr><tr><td>' + self.language.getString(user.language, 'login_email') + '</td><td>' + dataset.register_email + '</td></tr><tr><td>' + self.language.getString(user.language, 'login_vorname') + '</td><td>' + dataset.register_vorname + '</td></tr><tr><td>' + self.language.getString(user.language, 'login_nachname') + '</td><td>' + dataset.register_nachname + '</td></tr><tr><td>' + self.language.getString(user.language, 'login_location') + '</td><td>' + dataset.register_location + '</td></tr><tr><td>' + self.language.getString(user.language, 'login_start') + '</td><td>' + dataset.register_start + '</td></tr><tr><td>' + self.language.getString(user.language, 'login_stop') + '</td><td>' + dataset.register_stop + '</td></tr></table>' // html body
           }
           self.transporter.sendMail(mailOptions, function (error, info) {
             if (error) {
               console.error(error)
