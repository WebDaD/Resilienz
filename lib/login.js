 ...
 */
 ...
 function Login (database, language, bcrypt, saltfile, secret) {
   var self = {}
   self.saltRounds = 10
   self.secret = secret
 ...
 function reset (email, callback) {
   ...
   self.createToken(email, function (token) {
     var mailOptions = {
       ...
       subject: '[Resilienz-App] RESET Password', // Subject line
       html: self.language.getString('en', 'reset_mail_text') + '<br/><a href="LINK/set-pwd?resetToken=' + token + '&email=' + email + '">Click me</a>' // html body
     }
