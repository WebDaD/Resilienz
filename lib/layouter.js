  ...
  self.database.getLayoutInfo(actionid, page, function (error, information) {
    if (error) {
      ...
      for (var i = 0; i < information.length; i++) {
        ...
        var bg = self.layoutpath + '/' + language + '/' + info.background
        var cmd = ''
        if (info.spin !== 0) {
          ...
          '-affine ' + Math.cos(info.spin * Math.PI / 180.0) + ',' + Math.sin(info.spin * Math.PI / 180.0) + ',-' + Math.sin(info.spin * Math.PI / 180.0) + ',' + Math.cos(info.spin * Math.PI / 180.0) +
