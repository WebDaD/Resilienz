          ...
          if (page === 2) {
            ...
            for (var j = 0; j < information.length; j++) {
              ...
              if (info.image === '') {
                ...
                var imgname = info.image
                info.image = self.imagepath + '/' + info.image
                var rscmd = 'convert ' + info.image + ' -resize ' + info.width + 'x' + info.height + ' -background white -compose Copy -gravity center -extent ' + info.width + 'x' + info.height + ' /tmp/' + imgname
                console.log(rscmd)
                output = execSync(rscmd, true)
                info.image = '/tmp/' + imgname
              }
              var bg = ''
              if (j === 0) {
