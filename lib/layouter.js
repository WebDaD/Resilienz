      ...
      self.database.getLayoutInfo(actionid, page, function (error, information) {
        if (error) {
          ...
          if (page === 2) {
            ...
            // check if any images for page. if not callback (null, false)
            var c = 0
            for (var j = 0; j < information.length; j++) {
              if (information[j].value === '') {
                c++
              }
            }
            if (c === information.length && (page === 1 || page === '1')) { // if no images on page and page is not page 1, return false
              return callback(null, false)
            }
            for (var i = 0; i < information.length; i++) {
              var info = information[i]
              if (info.type === '') {
                if (info.possible_type === 'both') {
                  info.type = 'image'
                } else {
                  info.type = info.possible_type
                }
              }
              var bg = ''
              if (i === 0) {
              ...
              if (info.type === 'image') {
                // image
                if (info.value === '') {
                  info.value = self.layoutpath + '/placeholder_print.png'
                } else {
                  var imgname = info.value
                  info.value = self.imagepath + '/' + info.value
                  var rscmd = 'convert ' + info.value + ' -resize ' + info.width + 'x' + info.height + ' -background white -compose Copy -gravity center -extent ' + info.width + 'x' + info.height + ' /tmp/' + imgname
                  console.log(rscmd)
                  output = execSync(rscmd, true)
                  info.value = '/tmp/' + imgname
                }
                cmd = ''
                if (info.spin !== 0) {
                  cmd = 'convert ' + bg + ' null: \\( ' + info.value + ' -sample ' + info.width + 'x' + info.height + '! -background None -rotate ' + info.spin + ' \\) -geometry \'+' + info.x + '+' + info.y + '\' -layers Composite ' + target
                } else {
                  cmd = 'composite -geometry \'' + info.width + 'x' + info.height + '!+' + info.x + '+' + info.y + '\' ' + info.value + ' ' + bg + ' ' + target
                }
                console.log(cmd)
                output = execSync(cmd, true)
              } else {
                // text
                var text = info.value.split('|')[1]
                // remove 10% from width / height and add half value to x / y for centering
                var w10 = info.width / 10
                var h10 = info.height / 10
                var wnew = info.width - w10
                var hnew = info.height - h10
                var nx = info.x - (w10 / 2)
                var ny = info.y - (h10 / 2)
                cmd = 'convert -font Helvetica -pointsize 200 -background white -fill \'#1e487a\' -gravity northwest -geometry \'' + wnew + 'x' + hnew + '!+' + nx + '+' + ny + '\' caption:"' + text + '" ' + bg + ' +swap -gravity northwest -composite  ' + target
                console.log(cmd)
                output = execSync(cmd, true)
              }
            }
...
function printNumbers (target, page) {
  ...
  var cmd2 = 'convert ' + target + ' -font Helvetica -pointsize 24 -gravity southeast -stroke \'#000C\' -strokewidth 2 -annotate +50+10 \'' + (page + 1) + '\' -stroke none -fill white -annotate +50+10 \'' + (page + 1) + '\' ' + target
  var output2 = execSync(cmd2, true)
