      ...
      self.database.getLayoutInfo(actionid, page, function (error, information) {
        if (error) {
          ...
          if (page === 2) {
            ...
            cmd = 'cp ' + '/' + language + '/16_ruecken.png' + self.pagespath + '/' + actionid + '/' + page + '.png'
            output = execSync(cmd)
          } else {
            for (var i = 0; i < information.length; i++) {
              ...
              cmd = ''
              if (info.spin !== 0) {
                cmd = 'composite -geometry ' + info.image + ' \'' + info.width + 'x' + info.height + '!+' + info.x + '+' + info.y + '\' ' +
                ' -background None -rotate ' + info.spin +
                ' ' + bg + ' ' + target
              } else {
