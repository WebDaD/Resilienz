...
function addImage (actionid, page, positionid, name, callback) {
  ...
  self.connection.query(sql, function (error, results, fields) {
    if (error) {
      ...
      self.connection.query(sql, function (error, results2, fields) {
        if (error) {
          callback(error)
        } else {
...
function addUser (data, callback) {
  ...
  self.getLanguageIDFromKey(data.register_language, function (error, languagesid) {
    if (error) {
      ...
      self.connection.query('INSERT INTO user (vorname, nachname, email, password, languages_id) VALUES("' + data.register_vorname + '", "' + data.register_nachname + '", "' + data.register_email + '", "' + data.password + '","' + languagesid + '")', function (error, results, fields) {
        if (error) {
          callback(error)
        } else {
...
function getLanguageIDFromKey (key, callback) {
  var self = this
  self.connection.query('SELECT id FROM languages WHERE lang_key="' + key + '"', function (error, results, fields) {
    if (error) {
      callback(error)
    } else {
      if (results.length !== 1) {
