...
function getBackgroundImage (page, callback) {
  var self = this
  self.connection.query({sql: 'SELECT background FROM resilienz.layout_backgrounds WHERE page=' + page, timeout: 360000}, function (error, results, fields) {
    if (error) {
      callback(error)
    } else {
...
function getLayoutImagesByActionPage (actionid, page, callback) {
  var self = this
  self.connection.query({sql: 'SELECT poa.position_id AS id, poa.position_name AS name, poa.height, poa.width, poa.x, poa.y, poa.spin, iop.image, poa.layouts_id, l.name AS layout_name, l.symbol AS layout_symbol FROM layouts l, positions_on_actions poa LEFT JOIN images_on_positions iop ON poa.action_has_layouts_id=iop.actions_has_layouts_id AND iop.positions_id=poa.position_id WHERE l.id=poa.layouts_id and poa.actions_id=' + actionid + ' and poa.page=' + page, timeout: 360000}, function (error, results, fields) {
    if (error) {
      callback(error)
    } else {
      var layout = {}
...
function getPositionImage (actionid, positionid, callback) {
  var self = this
  self.connection.query({sql: 'SELECT iop.image FROM images_on_positions iop, actions_has_layouts ahl WHERE ahl.id = iop.actions_has_layouts_id AND ahl.actions_id=' + actionid + ' AND iop.positions_id=' + positionid, timeout: 360000}, function (error, results, fields) {
    if (error) {
      callback(error)
    } else {
...
function getImage (actionid, page, positionid, callback) {
  var self = this
  self.connection.query({sql: 'SELECT iop.image FROM images_on_positions iop, actions_has_layouts ahl WHERE ahl.id = iop.actions_has_layouts_id AND iop.positions_id=' + positionid + ' AND ahl.actions_id=' + actionid + ' AND ahl.page=' + page, timeout: 360000}, function (error, results, fields) {
    if (error) {
      callback(error)
    } else {
...
function addImage (actionid, page, positionid, name, callback) {
  ...
  self.connection.query({sql: sql, timeout: 360000}, function (error, results, fields) {
    if (error) {
      ...
      sql = 'INSERT INTO images_on_positions (actions_has_layouts_id, positions_id, image) VALUES (' + results[0].id + ',' + positionid + ',"' + name + '")'
...
function removeImage (name, callback) {
  var self = this
  self.connection.query({sql: 'DELETE FROM images_on_positions WHERE image="' + name + '"', timeout: 360000}, function (error, results, fields) {
    if (error) {
      callback(error)
    } else {
...
function getUser (email, callback) {
  var self = this
  self.connection.query({sql: 'SELECT a.id AS action_id, u.id AS id, u.email As email, u.admin AS admin, u.password AS password, l.lang_key FROM user u  LEFT JOIN actions a ON a.user_id=u.id LEFT JOIN languages l ON l.id = u.languages_id WHERE u.email="' + email + '" LIMIT 1', timeout: 360000}, function (error, results, fields) {
    if (error) {
      callback(error)
    } else {
    ...
    if (error) {
      ...
      self.connection.query({sql: 'INSERT INTO user (vorname, nachname, email, password, languages_id) VALUES("' + data.register_vorname + '", "' + data.register_nachname + '", "' + data.register_email + '", "' + data.password + '","' + languagesid + '")', timeout: 360000}, function (error, results, fields) {
        if (error) {
          ...
          self.connection.query({sql: 'INSERT INTO actions (location, start_time, end_time, finalized, user_id) VALUES("' + data.register_location + '", "' + data.register_start + '", "' + data.register_stop + '", "0","' + results.insertId + '")', timeout: 360000}, function (error, results2, fields) {
            if (error) {
              callback(error)
            } else {
...
function getActions (callback) {
  var self = this
  self.connection.query({sql: 'SELECT email, language, location, start_time, end_time, finalized, book FROM actionList', timeout: 360000}, function (error, results, fields) {
    if (error) {
      callback(error)
    } else {
      callback(null, results)
...
function getAction (id, callback) {
  var self = this
  self.connection.query({sql: 'SELECT email, language, location, start_time, end_time, finalized, book FROM actionList WHERE user_id=' + id, timeout: 360000}, function (error, results, fields) {
    if (error) {
      callback(error)
    } else {
      callback(null, results[0])
...
function getTranslations (callback) {
  var self = this
  self.connection.query({sql: 'SELECT language_key, string_key, translation FROM resilienz.language_translations ORDER BY language_key ASC, string_key ASC', timeout: 360000}, function (error, results, fields) {
    if (error) {
      callback(error)
    } else {
      var translations = {}
...
function getLayoutInfo (actionid, page, callback) { // return array of information.{width, height, x, y, imagefile, layoutfile}
  ...
  if (page === 0 || page === 2 || page === 48) {
    ...
    var sql = 'SELECT poa.width, poa.height, poa.x, poa.y, poa.spin, poa.layouts_id, iop.image, lb.background FROM positions_on_actions poa LEFT JOIN images_on_positions iop ON iop.actions_has_layouts_id=poa.action_has_layouts_id AND iop.positions_id=poa.position_id JOIN layout_backgrounds lb WHERE lb.page=' + page + ' AND poa.actions_id=' + actionid + ' AND poa.page=' + page
