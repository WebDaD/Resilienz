    ...
    if (error) {
      ...
      for (var i = 0; i < results.length; i++) {
        ...
        if (!arrayHasObjectWithID(thisCat.layouts, results[i].layouts_id)) { // layout must be created
          ...
          layout.display = '<span class="icon-layout_symbols_' + results[i].layoutSymbol + '"> ' + results[i].layoutName + '</span'
          layout.positions = []
...
function getLayoutImagesByActionPage (actionid, page, callback) {
  var self = this
  self.connection.query({sql: 'SELECT poa.position_id AS id, poa.position_name AS name, poa.height, poa.width, poa.x, poa.y, poa.spin, iop.image, poa.layouts_id, l.name AS layout_name FROM layouts l, positions_on_actions poa LEFT JOIN images_on_positions iop ON poa.action_has_layouts_id=iop.actions_has_layouts_id AND iop.positions_id=poa.position_id WHERE l.id=poa.layouts_id and poa.actions_id=' + actionid + ' and poa.page=' + page, timeout: 60000}, function (error, results, fields) {
    if (error) {
      callback(error)
    } else {
      var layout = {}
