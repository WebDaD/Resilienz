...
function getCategoriesFull (callback) {
  var self = this
  self.connection.query('', function (error, results, fields) { // TODO: create SELECT Statement
    if (error) {
      callback(error)
    } else {
      var categories = [] // TODO: build object ({id, sort: -1, name: '', pages: -1, layouts: []}) with allowed layouts and their positions and pages (show real pages eg 32, but only the left ones!)
...
function addImage (actionid, page, positionid, name, callback) {
  var self = this
  self.connection.query('', function (error, results, fields) { // TODO: create INSERT Statement
    if (error) {
      callback(error)
    } else {
      callback(null, results[0].name) // TODO: callback id. not needed, but do it anyways
    }
  })
}
...
function saveLayout(actionid, page, layoutid, callback) {
  var self = this
  self.connection.query('', function (error, results, fields) { // TODO: write sql
    if (error) {
      callback(error)
    } else {
      callback(null, results)
