...
function getCategoriesFull (callback) {
  var self = this
  self.connection.query('SELECT c.id, c.name, c.pages, c.start_page as startpage, c.ordering, c.lang_string_key, chl.layouts_id, p.id as position_id, p.width, p.height FROM categories c, categories_has_layouts chl, positions p WHERE c.id=chl.categories_id AND chl.layouts_id=p.layouts_id', function (error, results, fields) {
    if (error) {
      callback(error)
    } else {
      var categories = []
