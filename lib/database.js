...
function getLayoutImagesByActionPage (actionid, page, callback) {
  var self = this
  self.connection.query({sql: 'SELECT p.id, p.name, p.height, p.width, p.x, p.y, p.spin, iop.image, l.id as layouts_id, l.name as layout_name FROM layouts l, resilienz.actions_has_layouts ahl, resilienz.positions p LEFT JOIN images_on_positions iop ON iop.positions_id=p.id WHERE p.layouts_id=ahl.layouts_id AND l.id = p.layouts_id AND ahl.actions_id=' + actionid + ' AND ahl.page=' + page, timeout: 60000}, function (error, results, fields) {
    if (error) {
      callback(error)
    } else {
      var layout = {}
...
function updateBookStatus (actionid, status, callback) {
  var self = this
  self.connection.query({sql: 'UPDATE action SET book="' + status + '" WHERE id=' + actionid, timeout: 60000}, function (error, results, fields) {
    if (error) {
      callback(error)
    } else {
      if (results.affectedRows === 1) {
...
function getActions (callback) {
  var self = this
  self.connection.query({sql: 'SELECT email, language, location, start_time, end_time, finalized FROM actionList', timeout: 60000}, function (error, results, fields) {
    if (error) {
      callback(error)
    } else {
      callback(null, results)
...
function getAction (id, callback) {
  var self = this
  self.connection.query({sql: 'SELECT email, language, location, start_time, end_time, finalized FROM actionList WHERE user_id=' + id, timeout: 60000}, function (error, results, fields) {
    if (error) {
      callback(error)
    } else {
      callback(null, results[0])
