...
function getActionsForUser (id, callback) {
  var self = this
  self.connection.query({sql: 'SELECT email, language, location, start_time, end_time, finalized, book, comment, last_change, comment FROM actionList WHERE user_id=' + id, timeout: 360000}, function (error, results, fields) {
    if (error) {
      callback(error)
    } else {
      callback(null, results)
...
function getAction (id, callback) {
  var self = this
  self.connection.query({sql: 'SELECT email, language, location, start_time, end_time, finalized, book, comment, last_change, comment FROM actionList WHERE active=1 AND user_id=' + id, timeout: 360000}, function (error, results, fields) {
    if (error) {
      callback(error)
    } else {
      callback(null, results[0])
