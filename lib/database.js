...
function getPositionImage (actionid, positionid, callback) {
  var self = this
  self.connection.query({sql: 'SELECT iop.value FROM objects_on_positions iop, actions_has_layouts ahl WHERE ahl.id = iop.actions_has_layouts_id AND ahl.actions_id=' + actionid + ' AND iop.positions_id=' + positionid, timeout: 360000}, function (error, results, fields) {
    if (error) {
      callback(error)
    } else {
...
function saveTextOnPosition (actionid, page, positionid, text, callback) {
  ...
  self.connection.query({sql: sql, timeout: 60000}, function (error, results, fields) {
    if (error) {
      ...
      sql = 'INSERT INTO objects_on_positions (actions_has_layouts_id, positions_id, value, type) VALUES (' + results[0].id + ',' + positionid + ',"' + text + '", "text")'
      self.connection.query({sql: sql, timeout: 60000}, function (error, results2, fields) {
        if (error) {
          callback(error)
        } else {
