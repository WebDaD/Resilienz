    ...
    if (error) {
      ...
      for (var i = 0; i < results.length; i++) {
        ...
        if (!categories[c]) {
          ...
          position.image = null
          lay.positions.push(position)
          cat.layouts.push(lay)
          categories.push(cat)
        } else {
          if (!categories[c].layouts[l]) {
            var layout = {}
            layout.id = results[i].layouts_id
            layout.name = results[i].layoutName
            layout.positions = []
            var pos = {}
            pos.id = results[i].position_id
            pos.width = results[i].width
            pos.height = results[i].height
            pos.image = null
            layout.positions.push(pos)
            categories[c].layouts.push(layout)
          } else {
            var posis = {}
            posis.id = results[i].position_id
            posis.width = results[i].width
            posis.height = results[i].height
            posis.image = null
            categories[c].layouts[l].positions.push(pos)
          }
        }
      }
