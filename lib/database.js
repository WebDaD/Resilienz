...
function getLayoutImagesByActionPage (actionid, page, callback) {
  var self = this
  self.connection.query({sql: 'SELECT poa.position_id AS id, poa.position_name AS name, poa.height, poa.width, poa.x, poa.y, poa.spin, iop.image, poa.layouts_id, l.name AS layout_name, l.symbol AS layout_symbol FROM layouts l, positions_on_actions poa LEFT JOIN images_on_positions iop ON poa.action_has_layouts_id=iop.actions_has_layouts_id AND iop.positions_id=poa.position_id WHERE l.id=poa.layouts_id and poa.actions_id=' + actionid + ' and poa.page=' + page, timeout: 60000}, function (error, results, fields) {
    if (error) {
      callback(error)
    } else {
      var layout = {}
...
function getPositionImage (actionid, positionid, callback) {
  var self = this
  self.connection.query({sql: 'SELECT iop.image FROM images_on_positions iop, actions_has_layouts ahl WHERE ahl.id = iop.actions_has_layouts_id AND ahl.actions_id=' + actionid + ' AND iop.positions_id=' + positionid, timeout: 60000}, function (error, results, fields) {
    if (error) {
      callback(error)
    } else {
...
function getImage (actionid, page, positionid, callback) {
  var self = this
  self.connection.query({sql: 'SELECT iop.image FROM images_on_positions iop, actions_has_layouts ahl WHERE ahl.id = iop.actions_has_layouts_id AND iop.positions_id=' + positionid + ' AND ahl.actions_id=' + actionid + ' AND ahl.page=' + page, timeout: 60000}, function (error, results, fields) {
    if (error) {
      callback(error)
    } else {
...
function removeImage (name, callback) {
  var self = this
  self.connection.query({sql: 'DELETE FROM images_on_positions WHERE image="' + name + '"', timeout: 60000}, function (error, results, fields) {
    if (error) {
      callback(error)
    } else {
...
function getEmailByAction (actionId, callback) {
  var self = this
  self.connection.query({sql: 'SELECT email FROM userList WHERE action_id=' + actionId, timeout: 60000}, function (error, results, fields) {
    if (error) {
      callback(error)
    } else {
