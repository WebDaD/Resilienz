...
function Database (config) {
  ...
  self.uptimeAction = uptimeAction
    ...
    if (error) {
      ...
      self.connection.query({sql: sql, timeout: 60000}, function (error, results2, fields) {
        if (error) {
          ...
          self.uptimeAction(actionid, function (error, result) {
            if (error) {
              callback(error)
            } else {
...
function getUsers (callback) {
  var self = this
  self.connection.query({sql: 'SELECT id, action_id, vorname, nachname, email, language, location FROM userList', timeout: 360000}, function (error, results, fields) {
    if (error) {
      callback(error)
    } else {
      callback(null, results)
...
function deleteUser (userID, actionID, callback) {
  var self = this
  self.connection.query({sql: 'DELETE FROM `objects_on_positions` WHERE `actions_has_layouts_id` IN (SELECT id FROM `actions_has_layouts` WHERE `actions_id` = '+actionID+')', timeout: 360000}, function (error, results, fields) {
    if (error) {
      callback(error)
    } else {
      self.connection.query({sql: 'DELETE FROM `actions_has_layouts` WHERE `actions_id` = ' + actionID, timeout: 360000}, function (error, results, fields) {
        if (error) {
          callback(error)
        } else {
          self.connection.query({sql: 'DELETE FROM `actions` WHERE `id` = ' + actionID, timeout: 360000}, function (error, results, fields) {
            if (error) {
              callback(error)
            } else {
              self.connection.query({sql: 'DELETE FROM `user` WHERE `id` = ' + userID, timeout: 360000}, function (error, results, fields) {
                if (error) {
                  callback(error)
                } else {
                  callback(null, ' user with id ' + userID + ' and action ' + actionID + ' removed')
                }
              })
            }
          })
...
function actionFinalize (id, callback) {
  ...
  self.connection.query({sql: 'UPDATE actions SET finalized=1 WHERE id=' + id, timeout: 360000}, function (error, results, fields) {
    if (error) {
      ...
      if (results.affectedRows === 1) {
        self.uptimeAction(id, function (error, result) {
          if (error) {
            callback(error)
          } else {
            callback(null, true)
...
function getActionsForUser (id, callback) {
  var self = this
  self.connection.query({sql: 'SELECT action_id AS id, email, language, location, start_time, end_time, finalized, book, comment, last_change, active FROM actionList WHERE user_id=' + id, timeout: 360000}, function (error, results, fields) {
    if (error) {
      callback(error)
    } else {
      callback(null, results)
...
function switchAction (userId, actionId, callback) {
  var self = this
  self.connection.query({sql: 'UPDATE actions SET active=0 WHERE user_id=' + userId, timeout: 360000}, function (error, results, fields) {
    if (error) {
      callback(error)
    } else {
      if (results.affectedRows > 0) {
        self.connection.query({sql: 'UPDATE actions SET active=1, last_change=now() WHERE user_id=' + userId + ' AND id=' + actionId, timeout: 360000}, function (error, results, fields) {
          if (error) {
            callback(error)
          } else {
            if (results.affectedRows === 1) {
              ...
              callback({msg: 'Error, updated ' + results.affectedRows + ' rows for user_id ' + userId + ' and actionID ' + actionId}) // eslint-disable-line standard/no-callback-literal
            }
          }
        })
      } else {
        callback({msg: 'Error, updated ' + results.affectedRows + ' rows for user_id ' + userId}) // eslint-disable-line standard/no-callback-literal
      }
    }
  })
}
function addAction (userId, comment, callback) {
  var self = this
  self.connection.query({sql: 'INSERT INTO actions (location, start_time, end_time, finalized, user_id, active, comment) VALUES("-", NOW(), NOW(), "0","' + userId + '", "0", "' + comment + '")', timeout: 360000}, function (error, results2, fields) {
    if (error) {
      callback(error)
    } else {
      // insert default layouts into actions_has_layouts
        ...
        if (error) {
          ...
          self.connection.query({sql: 'INSERT INTO actions_has_layouts (actions_id, layouts_id, page) VALUES ' + ahlValues, timeout: 360000}, function (error, results3, fields) {
            if (error) {
              ...
              callback(null, {id: userId, action_id: results2.insertId})
            }
          })
        }
      })
...
function uptimeAction (actionId, callback) {
  this.connection.query({sql: 'UPDATE actions SET last_change=now() WHERE id=' + actionId, timeout: 360000}, function (error, results, fields) {
    if (error) {
      callback(error)
    } else {
      if (results.affectedRows === 1) {
        ...
        callback({msg: 'Error, updated ' + results.affectedRows + ' rows for actionID ' + actionId}) // eslint-disable-line standard/no-callback-literal
      }
    }
  })
}
