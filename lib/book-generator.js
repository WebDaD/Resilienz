 ...
 */
 ...
 function createPage (layouter, actionid, language, pagespath, page, callback) {
   layouter.createPage(actionid, language, page, function () {
     ...
     if (split) {
       ...
       execSync('mv /tmp/splitted_0.png ' + pagespath + '/' + actionid + '/' + page + '.png')
       execSync('mv /tmp/splitted_1.png ' + pagespath + '/' + actionid + '/' + (page + 1) + '.png')
 ...
 function composeFiles (layouter, pagespath, actionid, language, maxPages, callback) {
   async.times(maxPages, function (n, next) {
     if (n !== 1 && n % 2 !== 0) {
       next(null, '')
     } else {
       createPage(layouter, actionid, language, pagespath, n, function (error, parameter) {
