    ...
    if (error) {
      ...
      if (objects.length === 0) {
        if (category === '1') {
          ...
          fs.unlinkSync(path.join(self.pagespath, actionid) + '/' + category.padStart(2, '0') + '-tmp' + page.padStart(2, '0') + '.png')
          fs.unlinkSync(path.join(self.pagespath, actionid) + '/' + category.padStart(2, '0') + '-tmp' + (parseInt(page) + 1).toString().padStart(2, '0') + '.png')
          callback(null, true)
        }
      } else {
...
function printTextField (target, object) { // -> adds text to page
  ...
  var cmd = 'convert -font Helvetica -pointsize 12 -background white -fill \'#1e487a\' -gravity northwest -geometry \'' + object.width + 'x' + object.height + '!+' + object.x + '+' + object.y + '\' caption:"' + text + '" ' + target + ' +swap -gravity northwest -composite  ' + target
  console.log(cmd)
