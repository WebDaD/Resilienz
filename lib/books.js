...
function createPage (actionid, category, page, language, id, callback) { // -> creates one page
  var self = this
  self.database.getLayoutInfo(actionid, page, function (error, objects) {
    if (error) {
      return callback(error)
    } else {
      if (id === -1) { // -1 --> delete page
        ...
        if (!fs.existsSync(self.pagespath, actionid) + '/tmp/')){
          fs.mkdirSync(self.pagespath, actionid) + '/tmp/')
        }
        if (!fs.existsSync(self.pagespath, actionid) + '/pages/')){
          fs.mkdirSync(self.pagespath, actionid) + '/pages/')
        }
        if (!fs.existsSync(self.pagespath, actionid) + '/book/')){
          fs.mkdirSync(self.pagespath, actionid) + '/book/')
        }
        try {
        ...
        for (let index = 0; index < objects.length; index++) {
          ...
          if (id === -2 || id === object.id) { // -2 --> render all objects
...
function createIndex (actionid, language, pages, callback) { //  -> print indexpage (3) 01-03.png
  ...
  self.database.getCatNamesByLanguage(language, function (error, translations) {
    if (error) {
      ...
      copyFileSync(self.layoutpath + '/' + language + '/02_inhalt.png', self.pagespath + '/book/' + actionid + '/01-03.png')
      var startY = 1228
