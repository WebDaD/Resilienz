  ...
  self.database.getLayoutInfo(actionid, page, function (error, objects) {
    if (error) {
      ...
      if (objects.length === 0) {
        ...
        var target = path.join(self.pagespath, actionid) + ('0' + category).substring(2) + '-' + ('0' + page).substring(2) + '.png'
        fs.copyFileSync(background, target)
...
function printNumber (actionid, page, realNumber) { // -> prints number on page
  var target = path.join(this.pagespath, actionid) + page.cat + '-' + page.page + '.png'
...
function createIndex (actionid, language, pages, callback) { //  -> print indexpage (3) 02-01.png
  ...
  // pages array of objects: {page, cat, path, realNumber}
  // use database to get Strings
  // remember that there are the index page itself and startpage (no number) and the endpage
  // copy basefile (02_inhalt)
  // use printTextField to print first chapter to 02_01.png
  // use printTextField to print  chapter-list to 02_01.png
}
  ...
  fs.readdir(path.join(this.imagepath + actionid), function (error, files) {
    if (error) {
      ...
      pages.sort(function (a, b) {
        if (parseInt(a.cat) < parseInt(b.cat)) {
          return -1
        } else if (parseInt(a.cat) == parseInt(b.cat)) {
          if (parseInt(a.page) < parseInt(b.page)) {
            return -1
          } else if (parseInt(a.page) > parseInt(b.page)) {
            return 1
