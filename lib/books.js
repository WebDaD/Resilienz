  ...
  self.database.getLayoutInfo(actionid, page, function (error, objects) {
    if (error) {
      ...
      if (objects.length === 0) {
        ...
        var target = path.join(this.pagespath, actionid) + ('0' + category).substring(2) + '-' + ('0' + page).substring(2) + '.png'
        // FIXME: if file not there, copy cat background
...
function createIndex (actionid, language, pages, callback) { //  -> print indexpage (3) 02-01.png
  ...
  // use printTextField to print to 02_01.png
}
