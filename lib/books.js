  ...
  self.database.getLayoutInfo(actionid, page, function (error, objects) {
    if (error) {
      ...
      if (objects.length === 0) {
        ...
        execSync('convert ' + target + ' -crop 2127x3072+2128+0 ' + path.join(self.pagespath, actionid) + '/' + category.padStart(2, '0') + '-' + (parseInt(page) + 1).padStart(2, '0') + '.png')
