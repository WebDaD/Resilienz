...
function printNumber (actionid, page) { // -> prints number on page
  ...
  var cmd = 'convert ' + page.path + ' -font Helvetica -pointsize 180 -density 27.8x28.2 -gravity ' + gravity + ' -stroke \'#000C\' -strokewidth 8 -annotate ' + coordinates + ' \'' + page.realNumber + '\' -stroke none -fill white -annotate ' + coordinates + ' \'' + page.realNumber + '\' ' + page.path
  console.log(cmd)
  var output = execSync(cmd, true)
  if (output.stderr || output.stdout) {
...
function printAllNumbers (actionid, pages, callback) { // -> call printNumber for all pages
  for (let index = 0; index < pages.length; index++) {
    ...
    console.log(parseInt(page.cat) + ' - '+page.page + ' >> ' + (parseInt(page.cat) !== 1 && parseInt(page.cat !== 99)))
...
function createIndex (actionid, language, pages, callback) { //  -> print indexpage (3) 01-03.png
  ...
  self.database.getCatNamesByLanguage(language, function (error, translations) {
    if (error) {
      ...
      copyFileSync(self.layoutpath + '/' + language + '/02_inhalt.png', self.pagespath + '/' + actionid + '/01-03.png')
      var startY = 1228
      for (let index = 1; index < translations.length; index++) {
        const chapter = translations[index]
        if (chapter.id === 2) {
          self.printTextField(self.pagespath + '/' + actionid + '/01-03.png', { // use printTextField to print first chapter to 02_01.png
            width: 1524,
            height: 136,
            x: 160,
            y: 700,
          ...
          if (startpage) {
            self.printTextField(self.pagespath + '/' + actionid + '/01-03.png', { // use printTextField to print  chapter-list to 02_01.png
              width: 1524,
              height: 136,
              x: 160,
              y: startY,
...
function copyFixed (actionid, language, callback) { // -> copies pages 2 and the last into the folder
  copyFileSync(path.join(this.layoutpath + '/' + language + '/02_vorwort.png'), path.join(this.pagespath + '/' + actionid + '/01-02.png'))
