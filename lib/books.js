    ...
    if (error) {
      ...
      if (objects.length === 0) {
        if (category === '1') {
          ...
          self.removePage(actionid, category.padStart(2, '0') + '-' + page.padStart(2, '0'))
          callback(null, true)
        }
      } else {
    ...
    if (error) {
      ...
      for (let index = 1; index < translations.length; index++) {
        ...
        if (chapter.id === 2) {
          ...
          console.log(chapter)
          var startpage = pages.find(x => x.cat === chapter.id)
          if (startpage && startpage.length > 0) {
            var start = startpage[0].realNumber
            console.log('INDEX printing ' + start + ': ' + chapter.translation)
